...
import type { DocTendencias, UbicacionTendencias } from "@/lib/datos/tipos";
  ...
  if (u.estado === "sin_lista") {
    ...
    cuerpo = (
      ...
      <ol className="mt-3 space-y-1.5">
        {u.tendencias.map((t) => (
          <li key={t.puesto} className="flex items-baseline gap-3 text-cuerpo">
            <span className="w-6 shrink-0 text-right text-meta tabular-nums text-tinta-meta">
              {t.puesto}
            </span>
            <a
              href={t.url}
              ...
              className="min-w-0 break-words text-tinta-titulo transition-colors hover:text-chart-1-texto"
            >
              {t.nombre}
            </a>
            {t.volumen === undefined ? null : (
              <span className="ml-auto shrink-0 text-meta tabular-nums text-tinta-meta">
                {numero(t.volumen)} posts
              </span>
            )}
          </li>
        ))}
...
export function PanelTendencias({
  ...
  return (
    <Bisel interior="p-6 md:p-8">
      {sinToken ? (
        ...
        <>
          ...
          <p className="mt-2 max-w-[70ch] text-meta text-tinta-prosa">
            Es el ranking de X para cada ubicación, no una medida de la ciudad.{" "}
            {sinVolumen
      ...
      <p className="mt-10 text-meta text-tinta-prosa">{PIE}</p>
    </Bisel>
  );
}
