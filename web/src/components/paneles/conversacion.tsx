...
function Tema({ fila, dias }: { fila: FilaTema; dias: number }) {
  ...
  return (
    <li className="border-t border-white/[0.06] py-5 first:border-0 first:pt-0">
      <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
        <h4 className="text-[15px] tracking-tight text-white">{fila.tema}</h4>
        <span className="text-2xs tabular-nums text-white/50">
          {numero(fila.comentarios)} {pluralizar(fila.comentarios, "comentario", "comentarios")}
        </span>
      </div>
      <p className="mt-2 max-w-[70ch] text-sm leading-snug text-white/75">{frase}</p>
      {s !== undefined && clasificados > 0 ? (
        <div className="mt-3 max-w-md">
          <BarraSentimiento s={s} ariaLabel={frase} />
        </div>
      ) : null}
      {meta.length > 0 ? <p className="mt-2 text-2xs text-white/45">{meta.join("; ")}.</p> : null}
    </li>
  );
}
...
export function PanelConversacion({
  ...
  if (error !== undefined) {
    return <p className="text-sm text-white/55">No hay panel de conversación en este corte.</p>;
  }
  ...
  if (sinLlave) {
    cabeza = (
      <p className="max-w-[70ch] text-sm leading-relaxed text-white/60">
    ...
    cabeza = (
      <>
        <p className="max-w-[70ch] text-base leading-snug text-white/90">{frase}</p>
        {sentimiento !== undefined && clasificados > 0 ? (
  ...
  return (
    <Bisel opaco interior="p-6 md:p-8">
      ...
      {filas.length > 0 ? (
        <div className="mt-8">
          <h3 className="text-xs text-white/55">Por tema</h3>
          <ul className="mt-3">
      ...
      {figuras.length > 0 ? (
        <div className="mt-8">
          <h3 className="text-xs text-white/55">Figuras mencionadas en comentarios</h3>
